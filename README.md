...
![GPUSharingScriptDemo](https://github.com/lovetocode120/Interactive-Easy-GPU-PV-Vietnamese-Version/blob/main/GPUSharingScriptDemo.png?raw=true)
...
* Ảo hóa phải được bật trên bo mạch chủ (VT-x, AMD-V hoặc SVM Mode) và [tính năng Hyper-V phải được bật hoàn toàn](https://docs.microsoft.com/en-us/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v) trên hệ điều hành Windows 10/11 (có yêu cầu khởi động lại).  
...
4) Tìm thư mục đã giải nén mà mọi người đã download xong và chạy script có tên "GPUP-management.ps1" hoặc "GPUPartitionSharingToVM-VN" (đối với những ai chưa biết tiếng Anh), click chuột phải và chọn "Run with PowerShell". Chọn "Tạo máy ảo mới với GPU tăng tốc đồ họa" hay "Create new VM with GPU acceleration" và setting các thông số cần thiết. Script sẽ bắt đầu tạo máy ảo sau khi setting tham số xong, có thể mất 5-10 phút tùy thuộc vào hệ thống của mọi người.
